...
@IBDesignable open class RecordButton: NSView {

    private var isRecording = false
    private var roundView: NSView?
    ...
    private func setupRecordButtonView() {
        ...
        drawExternalCircle()
        drawRoundedButton()
//        self.addGestureRecognizer(NSTapGestureRecognizer(target: self, action: #selector(tappedView(_:))))
...
//                                  clockwise: true).cgPath
    ...
    private func drawRoundedButton() {
        ...
        roundView = NSView(frame: CGRect(x: self.frame.size.width/2-squareSide!/2,
                                         y: self.frame.size.height/2-squareSide!/2,
                                         width: squareSide!,
                                         height: squareSide!))
        roundView?.backgroundColor = NSColor.red
